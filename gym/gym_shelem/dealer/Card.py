...
class Card(object):
    ...
    def numeric_value(self):
        return (self.suit.value - 1) * len(VALUES) + self.value.value.normal_value

    @staticmethod
    def from_numeric_value(value):
        normal_value = value % len(VALUES)
        suit = int((value - normal_value) / len(VALUES)) + 1
        return Card(VALUES.from_normal_value(normal_value), SUITS.from_value(suit))
